...
var SimpleTable = function(tableId, options) {
	...
	var gatherHeadInfo = function(instance) {
		...
		if (ths.length > 0) {
			instance.ckhead = ths.filter(":[colname='checkbox']")
			...
			$.ajax({
				...
				success : function(data, textStatus, XMLHttpRequest) {
					...
					if (typeof instance.options.checkboxRenderer == 'function') {
						instance.options.checkboxRenderer(instance.ckcol,
								instance.ckhead);
					}
	...
	var resetPageInfo = function(instance, pageinfo) {
		...
		instance.pageinfo.totalPages = Math.ceil(pageinfo.totalRecords
				/ pageinfo.recordsPerPage);
	...
	var buildSelectHtml = function(instance, pageSizeDiv) {
		...
		if (pageSizeHtml.indexOf('{0}') >= 0) {
			...
			var words2 = pageSizeHtml
					.substring(pageSizeHtml.indexOf('{0}') + 3);
			...
			if (currPageSize <= 5) {
				option1.val(currPageSize).html(currPageSize).attr("selected",
						"selected");
				option2.val(currPageSize * 2).html(currPageSize * 2);
			} else {
				option1.val(currPageSize / 2).html(currPageSize / 2);
				option2.val(currPageSize).html(currPageSize).attr("selected",
						"selected");
			}
			...
			pageSize2.change(function(v) {
						var newPageSize = $(this).children('option:selected')
								.val();
	...
	var buildPageHtml = function(instance, div) {
		...
		if (pagingInfo.length > 0) {
			if (instance.pageinfo.totalRecords == 0) {
				...
				pagingInfo = pagingInfo.replace('{0}', 1
								+ (instance.pageinfo.currentPage - 1)
								* instance.options.pageSize);
				pagingInfo = pagingInfo.replace('{1}',
						(instance.pageinfo.currentPage - 1)
								* instance.options.pageSize
								+ instance.datas.length);
				pagingInfo = pagingInfo.replace('{2}',
						instance.pageinfo.totalRecords);
			}
			div.html(pagingInfo);
		}
	}
	...
	var addEventListener = function(instance) {
		if (instance.ckhead && instance.ckhead.length == 1
				&& instance.ckcol.length > 0) {
	...
	var addSortListener = function(instance, key) {
		for (var i = 0; i < instance.cols.length; i++) {
			...
			if (key == col.key) {
				col.element.addClass(instance.options.css.sortable).attr(
						'sortable', true).click(function() {
	...
	this.showColumn = function(colname) {
		for (var i = 0; i < this.cols.length; i++) {
			if (this.cols[i].key == colname) {
				...
				this.table.find('th[colname="' + colname + '"]').attr(
						'display', true).show();
				break;
			}
		}
...
var PMSTableAdapter = function() {
	...
	this.initParam = function(tableInstance) {
		var param = '';
		param += "requestString=";
		var manager = tableInstance.options.managerName
				? tableInstance.options.managerName
				: "queryManager";
		var method = tableInstance.options.methodName
				? tableInstance.options.methodName
				: "executeQuery";
